from sqlalchemy import Column, String, Integer, DateTime, Boolean, Text, DECIMAL, ForeignKey, JSON
...
class Company(Base):
    ...
    company_size = Column(String(50))  # e.g., '1-10', '11-50', '51-200', '201-500', '500+'
    location = Column(JSONB)  # {"city": "San Francisco", "state": "CA", "country": "USA"}
    description = Column(Text)
    founded_year = Column(Integer)
    revenue_range = Column(String(50))  # e.g., '$1M-$10M', '$10M-$50M'
    technology_stack = Column(JSONB)  # Array of technologies used
    social_media = Column(JSONB)  # {"linkedin": "url", "twitter": "url", "facebook": "url"}
    employee_count = Column(Integer)
    growth_signals = Column(JSONB)  # {"hiring": true, "funding": true, "expansion": false}
...
class Contact(Base):
    ...
    seniority_level = Column(String(50))  # e.g., 'Entry', 'Mid', 'Senior', 'Executive', 'C-Level'
...
class ScrapingJob(Base):
    ...
    job_type = Column(String(50), nullable=False)  # e.g., 'google_my_business', 'linkedin', 'website', 'directory'
    ...
    performance_metrics = Column(JSONB)  # {"avg_response_time": 1.5, "rate_limit_hits": 3}
    ...
    # Relationships
    scraped_data = relationship("ScrapedData", back_populates="scraping_job", cascade="all, delete-orphan")
...
class ScrapedData(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    job_id = Column(UUID(as_uuid=True), ForeignKey("scraping_jobs.id", ondelete="CASCADE"))
    ...
    source_type = Column(String(50), nullable=False)  # 'google_my_business', 'linkedin', 'website', 'directory'
    ...
    duplicate_of = Column(UUID(as_uuid=True), ForeignKey("scraped_data.id"))  # Reference to original if duplicate
...
class UserActivity(Base):
    ...
    activity_type = Column(String(50), nullable=False)  # 'login', 'scrape', 'export', 'view', 'update'
