...
from app.services.supabase_service import DatabaseError
...
class DatabaseUtils:
    ...
    def bulk_update(
        self, table_name: str, updates: List[Dict[str, Any]], id_field: str = "id"
    ) -> List[Dict[str, Any]]:
    ...
    def bulk_delete(
        self, table_name: str, ids: List[Union[str, UUID]], id_field: str = "id"
    ) -> int:
        """Delete multiple records by IDs."""
        try:
            str_ids = [str(id_val) for id_val in ids]
            response = (
                self.client.table(table_name).delete().in_(id_field, str_ids).execute()
            )
    ...
    def get_table_stats(
        ...
        try:
            ...
            return {
                ...
                "growth_rate": (
                    (recent_count / total_count * 100) if total_count > 0 else 0
                ),
            }
        except Exception as e:
    ...
    def cleanup_old_records(
        self, table_name: str, days_old: int = 30, date_field: str = "created_at"
    ) -> int:
    ...
    def archive_records(
        ...
        try:
            ...
            response = (
                self.client.table(table_name).select("*").in_("id", str_ids).execute()
            )
            records = response.data or []
            ...
            archive_response = (
                self.client.table(archive_table_name).insert(records).execute()
            )
            archived_count = len(archive_response.data) if archive_response.data else 0

            # Delete from original table
            delete_response = (
                self.client.table(table_name).delete().in_("id", str_ids).execute()
            )
    ...
    def optimize_table_performance(self, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if stats["growth_rate"] > 50:
                suggestions.append(
                    "High growth rate detected - consider data archiving strategy"
                )
            ...
            logger.error(
                f"Error analyzing table performance for {table_name}: {str(e)}"
            )
