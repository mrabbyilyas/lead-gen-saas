...
class MonitoringService:
    ...
    def check_database_health(self) -> ServiceHealth:
        ...
        try:
            ...
            return ServiceHealth(
                name="database", status=ServiceStatus.UNHEALTHY, error=str(e)
            )
    ...
    def check_redis_health(self) -> ServiceHealth:
        ...
        try:
            ...
            return ServiceHealth(
                name="redis", status=ServiceStatus.UNHEALTHY, error=str(e)
            )
    ...
    def check_celery_health(self) -> ServiceHealth:
        ...
        try:
            ...
            status = (
                ServiceStatus.HEALTHY if worker_count > 0 else ServiceStatus.DEGRADED
            )
            ...
            return ServiceHealth(
                name="celery", status=ServiceStatus.UNHEALTHY, error=str(e)
            )
        ...
        if self.redis_client:
            try:
                ...
                self.redis_client.setex(
                    error_key, 86400, str(error_info)
                )  # Store for 24 hours
