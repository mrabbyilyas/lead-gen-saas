...
class ProxyServer:
    ...
    def record_request(self, success: bool, response_time: float = 0.0, 
                      blocked: bool = False, rate_limited: bool = False) -> None:
    ...
    def needs_health_check(self) -> bool:
        ...
        return (
            time.time() - self.last_health_check > self.health_check_interval or
            self.status in [ProxyStatus.ERROR, ProxyStatus.BLOCKED]
        )
...
class ProxyManager:
    ...
    def record_request_result(self, proxy: ProxyServer, url: str, 
    ...
    def remove_proxy(self, host: str, port: int) -> bool:
        ...
        for i, proxy in enumerate(self.proxies):
            if proxy.config.host == host and proxy.config.port == port:
                ...
                domains_to_remove = [
                    domain for domain, mapped_proxy in self.domain_proxy_mapping.items()
