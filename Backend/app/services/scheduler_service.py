...
class SchedulerService:
    ...
    async def _comprehensive_health_check(self) -> None:
        ...
        try:
            ...
            logger.info(f"System health check completed - Score: {health_score}, Status: {status}")
            ...
            if critical_alerts:
                logger.warning(f"Found {len(critical_alerts)} critical alerts during health check")
                for alert in critical_alerts:
                    logger.warning(f"Critical Alert - {alert.get('service')}: {alert.get('message')}")
    ...
    async def _generate_daily_report(self) -> None:
        ...
        try:
            ...
            logger.info(f"Daily report generated - Health Score: {report['health_summary']['overall_score']}, "
        ...
        try:
            ...
            if hasattr(self.monitoring, 'performance_history'):
                ...
                if history_length > 100:
                    self.monitoring.performance_history = self.monitoring.performance_history[-100:]
                    logger.info(f"Cleaned up performance history: removed {history_length - 100} old entries")
            ...
            if self.monitoring.redis_client:
                try:
                    ...
                    if cleaned_keys > 0:
                        logger.info(f"Cleaned up {cleaned_keys} old error entries from Redis")
                        
                except Exception as redis_error:
                    logger.warning(f"Failed to cleanup Redis error entries: {redis_error}")
        ...
        try:
            ...
            if critical_alerts:
                ...
                for alert in critical_alerts:
                    ...
                    logger.critical(f"CRITICAL ALERT - {alert.get('service', 'unknown')}: "
            ...
            if high_alerts:
                ...
                for alert in high_alerts:
                    logger.warning(f"HIGH ALERT - {alert.get('service', 'unknown')}: "
                                 f"{alert.get('message', 'No message')}")
            
            logger.debug(f"Alert check completed - {len(alerts)} total alerts processed")
